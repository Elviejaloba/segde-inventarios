      ...
      <section className="rounded-[28px] border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-5 md:px-6 md:py-5">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3"><div className="rounded-2xl bg-sky-50 p-2.5 text-sky-700 shadow-sm"><ClipboardList className="h-5 w-5" /></div><div><h3 className="text-xl font-bold text-slate-950">Inventario</h3><p className="mt-1 text-sm text-slate-600">Relevá varias telas rápido, con sesión propia por operador y descarga final en Excel.</p></div></div>
            <div className="flex flex-wrap items-center gap-2" data-tour="rinde-inventory-export"><Button type="button" variant="outline" className="rounded-2xl" disabled={!inventoryPayload?.items.length} onClick={() => inventoryPayload && downloadInventoryExcel(inventoryPayload)}><Download className="mr-2 h-4 w-4" />Descargar Excel</Button><Button type="button" className="rounded-2xl bg-slate-900 hover:bg-slate-800" disabled={!currentSessionId || !inventoryItems.length || finalizeInventoryMutation.isPending} onClick={() => { if (window.confirm("¿Finalizar este inventario? Se descargará el Excel y la sesión dejará de estar activa.")) finalizeInventoryMutation.mutate(); }}>{finalizeInventoryMutation.isPending ? "Finalizando..." : "Finalizar inventario"}</Button></div>
          </div>
          <div className="grid gap-3 lg:grid-cols-[280px_minmax(0,1fr)]" data-tour="rinde-branch"><Card className="border-slate-200 shadow-none"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Building2 className="h-4 w-4 text-sky-700" />Sucursal</CardTitle><CardDescription>Usamos la misma fuente canónica del proyecto, incluida MCD Ciudad.</CardDescription></CardHeader><CardContent className="space-y-3 pt-0"><select value={selectedBranch} onChange={(event) => { setSelectedBranch(event.target.value as Branch | ""); resetInventoryForm(); }} className="h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-none outline-none focus:border-emerald-500"><option value="">Seleccionar sucursal...</option>{AVAILABLE_BRANCHES.map((branch) => <option key={branch} value={branch}>{branch}</option>)}</select><div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">{currentSessionId ? <>Sesión activa: <span className="font-medium text-slate-900">{currentSessionId.slice(0, 8)}</span></> : "Elegí una sucursal e iniciá un inventario activo para no mezclar relevamientos."}</div>{!currentSessionId ? <Button type="button" className="w-full rounded-2xl bg-emerald-600 hover:bg-emerald-700" disabled={!selectedBranch || createSessionMutation.isPending} onClick={() => { if (!selectedBranch) return; createSessionMutation.mutate(selectedBranch); }}>{createSessionMutation.isPending ? "Iniciando..." : "Iniciar inventario"}</Button> : null}</CardContent></Card><div className="space-y-3"><div className="hidden md:grid md:grid-cols-5 md:gap-3" data-tour="rinde-inventory-summary"><div className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Artículos</p><p className="mt-1 text-lg font-semibold text-slate-900">{inventorySummary.rowCount}</p></div><div className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Metros abiertos</p><p className="mt-1 text-lg font-semibold text-slate-900">{formatNumber(inventorySummary.openMeters)} m</p></div><div className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Metros cerrados</p><p className="mt-1 text-lg font-semibold text-slate-900">{formatNumber(inventorySummary.closedMeters)} m</p></div><div className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Rollos cerrados</p><p className="mt-1 text-lg font-semibold text-slate-900">{inventorySummary.closedRolls}</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Total</p><p className="mt-1 text-lg font-semibold text-emerald-700">{formatNumber(inventorySummary.totalMeters)} m</p></div></div><Accordion type="multiple" value={inventoryMobileSections} onValueChange={setInventoryMobileSections} className="rounded-[24px] border border-slate-200 bg-white px-4 shadow-sm md:hidden" data-tour="rinde-inventory-summary-mobile"><AccordionItem value="summary"><AccordionTrigger className="py-4 text-left text-sm font-semibold text-slate-900 hover:no-underline"><div><p>Resumen · {inventorySummary.rowCount} artículos · {formatNumber(inventorySummary.totalMeters)} m</p><p className="mt-1 text-xs font-normal text-slate-500">Tocá para ver abiertos, cerrados y detalle.</p></div></AccordionTrigger><AccordionContent><div className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700"><p>{formatNumber(inventorySummary.openMeters)} m abiertos · {formatNumber(inventorySummary.closedMeters)} m cerrados</p><p>{inventorySummary.closedRolls} rollos cerrados · Total {formatNumber(inventorySummary.totalMeters)} m</p></div></AccordionContent></AccordionItem></Accordion><Card className="border-slate-200 shadow-none"><CardHeader className="pb-3"><CardTitle className="text-base">Carga rápida</CardTitle><CardDescription>Rinde, peso y rollos: el resto se calcula solo para acelerar el relevamiento.</CardDescription></CardHeader><CardContent className="space-y-4 pt-0" data-tour="rinde-inventory-form"><div className="grid gap-3 xl:grid-cols-[minmax(260px,1.6fr)_140px_140px_minmax(180px,1fr)_auto]"><div className="relative"><Input value={inventorySearch} onFocus={() => setInventorySearchOpen(true)} onBlur={() => { inventorySearchBlurTimer.current = window.setTimeout(() => setInventorySearchOpen(false), 120); }} onChange={(event) => { setInventorySearch(event.target.value); setInventorySearchOpen(true); if (inventorySelectedRinde && normalize(event.target.value) !== normalize(getReferenceLabel(inventorySelectedRinde))) setInventorySelectedRinde(null); }} placeholder="Buscar rinde verificado por CDD" className="h-11 rounded-2xl border-slate-200 bg-white text-sm shadow-none" disabled={!currentSessionId || inventorySessionQuery.isLoading} />{inventorySearchOpen && inventorySuggestions.length > 0 ? <div className="absolute z-30 mt-2 max-h-56 w-full overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">{inventorySuggestions.map((item) => <button key={`${item.articleCode}-${item.referenceLabel || ""}`} type="button" className="flex w-full flex-col rounded-xl px-3 py-2 text-left hover:bg-emerald-50" onMouseDown={(event) => event.preventDefault()} onClick={() => { if (inventorySearchBlurTimer.current) window.clearTimeout(inventorySearchBlurTimer.current); handleSelectInventoryRinde(item); }}><span className="font-semibold text-slate-900">{getReferenceLabel(item) || item.articleCode}</span><span className="text-xs text-slate-500">{renderRindeMeta(item)}</span></button>)}</div> : null}</div><Input ref={pesoInputRef} value={inventoryPeso} onChange={(event) => setInventoryPeso(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); rollosInputRef.current?.focus(); rollosInputRef.current?.select(); } }} inputMode="decimal" placeholder="Peso (kg)" className="h-11 rounded-2xl" disabled={!currentSessionId || inventorySessionQuery.isLoading} /><Input ref={rollosInputRef} value={inventoryRollos} onChange={(event) => setInventoryRollos(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); upsertInventoryItemMutation.mutate(); } }} inputMode="numeric" placeholder="Rollos" className="h-11 rounded-2xl" disabled={!currentSessionId || inventorySessionQuery.isLoading} /><Input value={inventoryObservation} onChange={(event) => setInventoryObservation(event.target.value)} placeholder="Observación" className="h-11 rounded-2xl" disabled={!currentSessionId || inventorySessionQuery.isLoading} /><div className="flex flex-col gap-2"><Button type="button" className="h-11 rounded-2xl bg-emerald-600 hover:bg-emerald-700" disabled={!currentSessionId || upsertInventoryItemMutation.isPending || !inventorySelectedRinde} onClick={() => upsertInventoryItemMutation.mutate()}>{upsertInventoryItemMutation.isPending ? "Guardando..." : editingItemId ? "Guardar fila" : "+ Agregar"}</Button>{(editingItemId || inventorySelectedRinde || inventoryPeso || inventoryRollos !== "0" || inventoryObservation) ? <Button type="button" variant="outline" className="h-10 rounded-2xl" onClick={resetInventoryForm}><X className="h-4 w-4" /></Button> : null}</div></div><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_180px_180px]"><div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Referencia</p><p className="mt-1 truncate text-sm font-semibold text-slate-900">{inventorySelectedRinde ? getReferenceLabel(inventorySelectedRinde) || inventorySelectedRinde.articleCode : "Elegí un rinde"}</p></div><div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Metros abiertos</p><p className="mt-1 text-sm font-semibold text-slate-900">{formatNumber(inventoryCalculation.openMeters)} m</p></div><div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Metros cerrados</p><p className="mt-1 text-sm font-semibold text-slate-900">{formatNumber(inventoryCalculation.closedMeters)} m</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Total fila</p><p className="mt-1 text-sm font-semibold text-emerald-700">{formatNumber(inventoryCalculation.totalMeters)} m</p></div></div></CardContent></Card></div></div>
          {inventorySessionQuery.isLoading && currentSessionId ? <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">Cargando inventario activo...</div> : null}
          {inventorySessionQuery.isError && currentSessionId ? <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-6 text-sm text-rose-700">No pudimos recuperar la sesión activa. Probá iniciar un inventario nuevo.</div> : null}
          <div className="hidden md:block overflow-hidden rounded-[24px] border border-slate-200 bg-white" data-tour="rinde-inventory-list"><div className="max-h-[30rem] overflow-auto"><table className="min-w-full text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-slate-600"><tr className="border-b border-slate-200">{["#","Rinde","Ancho","Peso (kg)","Kg/m","Metros abiertos","Rollos cerrados","M/rollo","Metros cerrados","Total metros","Observación","Acciones"].map((label) => <th key={label} className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide">{label}</th>)}</tr></thead><tbody>{inventoryItems.length ? inventoryItems.map((item, index) => <tr key={item.id} className="border-b border-slate-100 align-top hover:bg-slate-50/70"><td className="px-3 py-3 text-slate-500">{index + 1}</td><td className="px-3 py-3 min-w-[180px]"><p className="font-semibold text-slate-900">{item.referenceLabel}</p><p className="text-xs text-slate-500">{item.articleCode}</p></td><td className="px-3 py-3 text-slate-700">{formatNumber(item.anchoCm, 0)} cm</td><td className="px-3 py-3 text-slate-700">{formatNumber(item.pesoKg)}</td><td className="px-3 py-3 text-slate-700">{formatNumber(item.kgPorMetro, 4)}</td><td className="px-3 py-3 font-medium text-slate-900">{formatNumber(item.metrosAbiertos)} m</td><td className="px-3 py-3 text-slate-700">{item.rollosCerrados}</td><td className="px-3 py-3 text-slate-700">{formatNumber(item.metrosReferencia)} m</td><td className="px-3 py-3 font-medium text-slate-900">{formatNumber(item.metrosCerrados)} m</td><td className="px-3 py-3 font-semibold text-emerald-700">{formatNumber(item.totalMetros)} m</td><td className="px-3 py-3 max-w-[220px] text-slate-600">{item.observacion || "—"}</td><td className="px-3 py-3"><div className="flex gap-2"><Button type="button" size="sm" variant="ghost" className="rounded-full px-3" onClick={() => handleEditInventoryItem(item)}><Pencil className="h-4 w-4" /></Button><Button type="button" size="sm" variant="ghost" className="rounded-full px-3 text-rose-600 hover:text-rose-700" onClick={() => handleDeleteInventoryItem(item)}><Trash2 className="h-4 w-4" /></Button></div></td></tr>) : <tr><td colSpan={12} className="px-4 py-10 text-center text-slate-500">Todavía no cargaste filas en este inventario.</td></tr>}</tbody></table></div></div>
          <div className="space-y-2 md:hidden" data-tour="rinde-inventory-list-mobile"><div className="max-h-[20rem] overflow-y-auto rounded-[24px] border border-slate-200 bg-white p-3">{inventoryItems.length ? inventoryItems.map((item) => <div key={item.id} className="flex items-start justify-between gap-3 border-b border-slate-100 py-3 last:border-b-0"><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate font-semibold text-slate-900">{item.referenceLabel}</p><span className="shrink-0 text-sm font-semibold text-emerald-700">{formatNumber(item.totalMetros)} m</span></div><p className="mt-1 text-xs text-slate-500">{formatNumber(item.pesoKg)} kg · {item.rollosCerrados} rollos</p>{item.observacion ? <p className="mt-1 text-xs text-slate-500">{item.observacion}</p> : null}</div><div className="flex shrink-0 items-center gap-1"><Button type="button" size="sm" variant="ghost" className="h-9 rounded-full px-3" onClick={() => handleEditInventoryItem(item)}><Pencil className="h-4 w-4" /></Button><Button type="button" size="sm" variant="ghost" className="h-9 rounded-full px-3 text-rose-600 hover:text-rose-700" onClick={() => handleDeleteInventoryItem(item)}><Trash2 className="h-4 w-4" /></Button></div></div>) : <p className="py-6 text-center text-sm text-slate-500">Todavía no cargaste filas en este inventario.</p>}</div></div>
        </div>
      </section>
